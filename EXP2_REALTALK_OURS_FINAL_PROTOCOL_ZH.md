...
这 519 是公开数据按论文规则重建得到的数量。论文没有公布 Table 2 的准确样本数，不能把 519 称为论文官方报告数字。
...
同一个人在同一 Session 中连续发送的消息气泡合并为一个语义 turn。不同 Session 不跨边界合并。
...
```text
H_t = Cb 选定片段中严格位于 M_t 之前的全部真实合并 turn
```
