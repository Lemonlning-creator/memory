...
def _markdown_summary(payload: Dict[str, Any]) -> str:
    ...
    lines.extend((
        ...
        "Rows marked `full_metric_reference` are completed all-10-conversation "
        "runs retained because they were strong on at least one metric. They are "
        "historical references, not outputs regenerated by the current sweep.",
        "",
        "| Method / Version | Role | N | Lexical | Semantic | Reflective | Grounding | Sentiment | Emotion | Intimacy | Empathy |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ))
    for row in results:
        role = str(row.get("comparison_role") or row.get("result_kind") or "reference")
        if not row.get("metrics"):
            lines.append(
                f"| {row['prompt_version']} | {role} | pending | "
                "- | - | - | - | - | - | - | - |"
            )
            continue
        values = []
        ...
        lines.append(
            "| " + " | ".join(
                [str(row["prompt_version"]), role, example_count] + values
            ) + " |"
        )
...
def summarize(
    *,
    ...
    reference_dirs: Sequence[str | Path],
    ...
    for reference_dir in reference_dirs:
        ...
        if reference_directory in existing_directories:
            continue
        reference = _load_result(reference_path)
        if reference is None:
            raise FileNotFoundError(
                "completed full-run metric reference not found: "
                f"{reference_path / 'evaluation' / 'table2_main_results.json'}"
            )
        reference["comparison_role"] = "full_metric_reference"
        reference["result_kind"] = "full_metric_reference"
        results.append(reference)
        existing_directories.add(reference_directory)
    ...
    payload = {
        ...
        "reference_dirs": [str(Path(path).resolve()) for path in reference_dirs],
        "cases": [case.case_id for case in cases],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    report.add_argument(
        ...
        default=[],
        help=(
            "Additional completed full-run result to retain as a metric-specific "
            "reference row. Repeat for multiple versions."
        ),
    )
