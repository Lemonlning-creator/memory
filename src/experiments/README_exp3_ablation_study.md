...
生成基线 Prompt 默认为 `v18_reflective_grounding_scores_only`：V18 联合门控的最终回复 system Prompt，且传给下一轮的上一轮状态只保留 `emotional_reaction`、`interpretation`、`exploration` 三个共情分数。三个子实验共用同一基线，需要更换生成基线时用 `--prompt-version` 显式指定。
