...
5. 行为层判定：当回复根据用户的内容兴趣、消费/娱乐偏好、习惯或长期行为模式，选择了特定话题、示例、推荐方向或行动路径时，应激活对应 behavior 字段。可以是语义等价或概括表达，不要求复述字段原文。
   - 例如，画像包含“偏好人工智能、认知科学、科研方法”，回复据此把话题引向 AI 的理解能力、论文、科研方法或相关工具，应激活 `behavior.content preferences`。
   - 仅当回复实际沿用该偏好来选择内容或建议时才激活；如果只是用户输入提及偏好、回复没有延续该方向，则不要激活。
