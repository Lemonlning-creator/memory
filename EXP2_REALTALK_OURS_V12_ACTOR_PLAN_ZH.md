...
冻结 V9 运行采用早期 Decision Schema，没有显式 `self_revelation_mode`，而 `missing_information` 位于同条冻结 `situation` 中。V12 对此使用确定性兼容映射：`answer/self-disclose/topic-shift` 映射为 `state-only`，其余动作映射为 `none`；追问信息从冻结 `situation.missing_information` 原样读取。映射不调用模型、不读取答案，也不根据自由文本关键词推断。
